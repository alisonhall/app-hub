...
// onSpawn (if given) is handed the child the moment it's spawned — see
// installNodeVersion's matching doc comment in lib/process-manager.js for
// why (letting stopApp() actually kill an in-flight install, not just mark
// the app STOPPED while the download keeps running untracked).
function installViaNvm(version, onSpawn) {
  return new Promise((resolve) => {
    ...
    let stderr = '';
    const child = spawn('bash', ['-c', script], { stdio: ['ignore', 'ignore', 'pipe'], detached: !IS_WINDOWS });
